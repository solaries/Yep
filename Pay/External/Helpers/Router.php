...
class Router
{
    ...
    const ID_KEY = 'id';
    const YEPPAY_API_ROOT = 'https://payment.yeppay.io/api/v1'; //https://payment.yeppay.io/api/v1/payment
    ...
    public function __construct($route, $yeppayObj)
    {
        $routes = $this->getAllRoutes($yeppayObj);
        ...
        $this->route_class = $this->getRouteClass($yeppayObj);
    ...
    private function getAllRoutes($yeppayObj)
    {
        return array_merge(static::$ROUTES, array_keys($yeppayObj->custom_routes));
    }

    private function getRouteClass($yeppayObj)
    {
        if (isset($yeppayObj->custom_routes[$this->route])) {
            return $yeppayObj->custom_routes[$this->route];
        }

        return 'Yep\\Pay\\External\\Routes\\' . ucwords($this->route);
    }
}
